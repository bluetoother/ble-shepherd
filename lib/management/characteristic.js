...
Characteristic.prototype.write = function (value, callback) {
    ...
    if (!value || (!_.isPlainObject(value) && !Buffer.isBuffer(value))) {
        deferred.reject(new Error('value must be an object or a buffer'));
    } else if ((this.prop & GATTDEFS.Prop.Write.value) || (this.prop & GATTDEFS.Prop.WriteNoRsp.value)) {
        ...
        ccBnp.gatt[cmd](this.ownerServ.ownerDev.connHdl, this.hdl, value, this.uuid).then(function () {
            ...
            if (err.errorCode === 5 || err.errorCode === 8 || err.errorCode === 15) {
                self.encryptAndReExec('write', value);
                deferred.resolve();
            } else {
                deferred.reject(err);
            }
...
Characteristic.prototype.setConfig = function (config, callback) {
    ...
    if (!_.isBoolean(config)) {
        deferred.reject(new Error('config must be a boolean'));
    }else if ((this.prop & GATTDEFS.Prop.Notif.value) || (this.prop & GATTDEFS.Prop.Ind.value)) {
        if (config === false) {
...
Characteristic.prototype.remove = function (callback) {
    ...
    bledb.remove('characteristic', this._id).then(function () {
        delete servChars[_.indexOf(servChars, self)];
        deferred.resolve();
    }).fail(function (err) {
        deferred.reject(err);
    }).done();
