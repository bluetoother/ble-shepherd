...
NwkScanner.prototype.setScanParams = function (setting, callback) { //time, interval, window
    ...
    if (!_.isPlainObject(setting)) {
        ...
        ccBnp.gap.setParam(2, (setting.time || 10240)).then(function (result) {
            self.scanParams.time = (setting.time || 10240);
            return ccBnp.gap.setParam(16, (setting.interval || 16));
        }).then(function (result) {
            self.scanParams.interval = (setting.interval || 16);
            return ccBnp.gap.setParam(17, (setting.window || 16));
        }).then(function (result) {
            self.scanParams.window = (setting.window || 16);
            deferred.resolve();
        }).fail(function (err) {
            deferred.reject(err);
        }).done(); 
...
NwkScanner.prototype.setLinkParams = function (setting, callback) { //interval, latency, timeout
    ...
    if (!_.isPlainObject(setting)) {
        ...
        ccBnp.gap.setParam(21, setting.interval || 80).then(function () {
            self.linkParams.interval = (setting.interval || 80);
            return ccBnp.gap.setParam(22, setting.interval || 80);
        }).then(function () {
            return ccBnp.gap.setParam(26, setting.latency || 0);
        }).then(function () {
            self.linkParams.latency = (setting.latency || 0);
            return ccBnp.gap.setParam(25, setting.timeout || 2000);
        }).then(function () {
            self.linkParams.timeout = (setting.timeout || 2000);
            deferred.resolve();
        }).fail(function (err) {
            deferred.reject(err);
        }).done();
