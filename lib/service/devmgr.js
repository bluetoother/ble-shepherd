...
BleDevice.prototype.linkParamUpdate = function (interval, connLatency, connTimeout, callback) {
	...
	if ( this.role === 'central' ) {
	    ccBnp.gap.setParam(21, interval).then(function () {
	        ccBnp.gap.setParam(22, interval).then(function () {
	            ccBnp.gap.setParam(26, connLatency).then(function () {
	                ccBnp.gap.setParam(25, connTimeout).then(function () {
	                    self.linkParams = {
							interval: interval, 
							latency: connLatency,
							timeout: connTimeout
...
BleDevice.prototype.getServs = function (callback) {
	...
	ccBnp.gatt.discAllPrimaryServices(this.connHandle).then(function (result) {
...
BleDevice.prototype.encrypt = function (setting, callback) {
	...
	if (!this.sm) { this.sm = new Secmdl(setting); }
	this.sm.init().then(function () {
		return this.sm.pairing();
	}).then(function () {
		if (self.sm.bond) { return this.sm.bond(); }
		return;
	}).then(function () {
		deferred.resolve();
	}).fail(function(err) {
