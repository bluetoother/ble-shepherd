...
bledb.getInfo = function (type, callback) {
 ...
 //            queryObj = {role: 'peripheral'};
 //            sortObj = {_id: 1};
	// 		// db.find({role: 'peripheral'}).sort({_id: 1}).exec(function (err, docs) {
	// 		// 	if (err) {
 //   //                  deferred.reject(err);
 //   //              } else {
 //   //                  deferred.resolve(docs);
 //   //              }
	...
	// 		break;
	// 	case 'service':
	// 		db.find({owner: {$exists: true}, ancestor: {$exists: false}}).sort({uuid: 1}).exec(function (err, docs) {
	// 			if (err) {
 //                    deferred.reject(err);
 //                } else {
 //                    deferred.resolve(docs);
 //                }
	...
	// 		break;
	// 	case 'characteristic':
	// 		db.find({owner: {$exists: true}, ancestor: {$exists: true}}).sort({uuid: 1}).exec(function (err, docs) {
	// 			if (err) {
 //                    deferred.reject(err);
 //                } else {
 //                    deferred.resolve(docs);
