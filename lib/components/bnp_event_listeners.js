...
listeners.discover = function (periphInfos) {
    ...
    _.forEach(periphInfos, function (periphInfo, index) {
        ...
        if (self._subModule === 'noble') 
            pInfo.advertisement = periphInfo.original.advertisement;
        else 
            pInfo.advertisement = periphInfo.advertisement || {};

        allowDev = Q.nbind(self.acceptDevIncoming, self);
        allowDevFns.push(allowDev(pInfo));

        // self.acceptDevIncoming(pInfo, function (err, accepted) {
        //     if (err) {
        //         self.emit('error', err);
        ...
        // if (self.onDiscover(pInfo) === false ) {
        //     if (_.isEmpty(pInfo.advertisement)) 
        //         self._controller.emit('connectErr');
        // } else {
        //     newPeriphInfos.push(periphInfo);
        // }
    });
