...
listeners.charNotif = function (charInfo) {
    ...
    if (!_.isNil(charInfo.servUuid)) {
        ...
        serv = _.find(periph.servs, function (serv) {
            return charInfo.charId > serv.startHandle && charInfo.charId < serv.endHandle;
        });
        servUuid = serv.uuid;
    ...
    if (!_.isEqual(oldVal, charInfo.value)) {
        path = 'servs.' + servUuid + '.chars.' + char.handle + '.value';
        this._periphBox.replace(periph._id, path, charInfo.value, function (err) {
            if (err) {
                ...
                emitObj.value = butil.objectDiff(oldVal, charInfo.value);
...
listeners.charChanged = function (charInfo) {
    var self = this,
        ...
        char = periph.findChar(charInfo.sid.uuid, charInfo.cid.handle),
        path = '',
        emitObj = {};
    ...
    path = 'servs.' + charInfo.servUuid + '.chars.' + char.handle + '.' + charInfo.type;
