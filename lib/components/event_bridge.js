...
bridge.handleBnpEvts = function (controller, subModule) {
    ...
    if (subModule === 'cc-bnp') {
        ...
        this.bnp = require('../../../bluetoother/cc-bnp');
...
bridge.handleNobleEvts = function (controller) {
    ...
    this.bnp._bindings.on('read', function (periphId, serviceUuid, characteristicUuid, characteristicHandle, value, isNotification) {
        var charUuid = '0x' + characteristicUuid;

        if (isNotification) {
            emitObj.type = 'attNoti';
            emitObj.periphId = '0x' + periphId;
            emitObj.charId = characteristicHandle;
