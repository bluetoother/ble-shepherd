...
bleutil.checkUuid = function (uuid, msg) {
    proving.stringOrNumber(uuid, msg);

    if (_.isString(uuid)) {
        uuid = _.lowerCase(uuid);
        if (!_.startsWith(uuid, '0x')) 
            uuid = '0x' + uuid;
    }
...
bleutil.checkAddr = function (addr, msg) {
    proving.string(addr, msg);

    addr = _.lowerCase(addr);
