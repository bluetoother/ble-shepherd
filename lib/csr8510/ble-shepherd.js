...
Manager.prototype.start = function (bleApp) {
...
Manager.prototype.setNwkParams = function (type, setting, callback) {
...
Manager.prototype.permitJoin = function (time, callback) {
    var self = this;
...
Manager.prototype.find = function (addrOrHdl) {
    var obj = {};
    ...
    return _.find(this.peripherals, obj);
};

Manager.prototype.regGattDefs = function (type, regObjs) {
    var enumName,
        uuid,
        errFlag = false,
        enumNewMem = {},
...
Manager.prototype._pausePeriph = function () {
    var deferred = Q.defer(),
        onlinePeriph;


    onlinePeriph = _.find(this.peripherals, {state: 'online'});
...
function scanHdlr (peripheral) {
    var updateCharFlag = false,
        connTimeout,
        connectPeriph = _.find(manager.peripherals, {addr: '0x' + peripheral.id});

    if (peripheral.id === '7cd1c3245f08' || peripheral.id === 'c869cd78823c') { return; }
...
function peripheralWrapper (peripheral) {
    ...
    newPeripheral = _.find(manager.peripherals, function (periph) {
        return '0x' + periph.id === peripheral.addr;
    });

    if (!newPeripheral) {
        newPeripheral = new Peripheral(peripheral);
        manager.peripherals.push(newPeripheral);
