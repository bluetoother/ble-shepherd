...
var BShepherd = function() {
    ...
    this.ban = function (permAddr, callback) {
        ...
        if (!this._blackOrWhite | this._blackOrWhite === 'white')  {
            callback(new Error('You should enable blocker with blacklist first.')); 
    ...
    this.unban = function (permAddr, callback) {
        ...
        if (!this._blackOrWhite | this._blackOrWhite === 'white')  {
            err = new Error('You should enable blocker with blacklist first.'); 
        } else {
    ...
    this.allow = function (permAddr, callback) {
        ...
        if (!this._blackOrWhite | this._blackOrWhite === 'black') {
            err = new Error('You should enable blocker with whitelist first.'); 
        } else if (!this.isWhiteListed(permAddr)) 
    ...
    this.disallow = function (permAddr, callback) {
        ...
        if (!this._blackOrWhite | this._blackOrWhite === 'black') {
            callback(new Error('You should enable blocker with whitelist first.')); 
        } else {
...
function scanHdlr (peripheral) {
    ...
    if (manager._blackOrWhite === 'white') {
        if (!manager.isWhiteListed('0x' + peripheral.id)) { 
            if (manager._permitState === 'on') { noble.startScanning(); }
            return; 
        }
    } else if (manager._blackOrWhite === 'black') {
        if (manager.isBlackListed('0x' + peripheral.id)) { 
            if (manager._permitState === 'on') { noble.startScanning(); }
            return; 
        }
    }
...
function syncPeriph (peripheral, updateFlag) {
    ...
    if (manager._syncSpinLock === 'off') {
        ...
        peripheral.discServsAndChars(updateFlag).then(function () {
            ...
            peripheral = examinePeriph(peripheral);
...
function examinePeriph (periph) {
    _.forEach(manager._plugins, function (examFunc, periphName) {
        var basicInfo = {
            ...
            fwRev: periph.servs['0x180a'].chars['0x2a26'].val.firmwareRev,
            hwRev: periph.servs['0x180a'].chars['0x2a27'].val.hardwareRev,
            swRev: periph.servs['0x180a'].chars['0x2a28'].val.softwareRev
        };
