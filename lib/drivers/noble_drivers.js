...
nobleDrivers.setLinkParams = function (setting) {
    var deferred = Q.defer(),
        interval = setting.interval || 0x0006,
    ...
    periph._original.discoverAllServicesAndCharacteristics(function (err, servs, chars) {
        if (err)
            ...
            _.forEach(servs, function (serv) {
                var servInfo = {
                        startHdl: serv._noble._bindings._gatts[serv._peripheralId]._services[serv.uuid].startHandle,
                        endHdl: serv._noble._bindings._gatts[serv._peripheralId]._services[serv.uuid].endHandle,
                        uuid: '0x' + serv.uuid,
                        original: serv,
                        chars: []
                    };
                ...
                _.forEach(serv.characteristics, function (char) {
                    var charInfo = {
                            uuid: '0x' + char.uuid,
                            hdl: char._noble._bindings._gatts[char._peripheralId]._characteristics[char._serviceUuid][char.uuid].valueHandle,
                            prop: char.properties,
                            original: char
                        };
                    servInfo.chars.push(charInfo);
                });
