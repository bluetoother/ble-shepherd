...
ccbnpDrivers.scan = function () {
    ...
    return ccbnp.gap.deviceDiscReq(3, 1, 0).then(function (result) {
        _.forEach(result.collector.GapDeviceInfo, function (devInfo) {
            advDatas[devInfo.addr] = advDatas[devInfo.addr] ? advDatas[devInfo.addr] : new Buffer([]);
            advDatas[devInfo.addr] = Buffer.concat([advDatas[devInfo.addr], devInfo.dataField]);
        });

        _.forEach(advDatas, function (advRawData, addr) {
            advDatas[addr] = parseAdvData(advRawData);
        });
        ...
        _.forEach(periphInfos, function (periphInfo) {
            periphInfo.advertisement = advDatas[periphInfo.addr];
        });
