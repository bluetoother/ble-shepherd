...
ccbnpDrivers.discAllServsAndChars = function (periph) {
    ...
    ccbnp.gatt.discAllPrimaryServices(periph.connHandle).then(function (result) {
        ...
        return ccbnp.gatt.readUsingCharUuid(periph.connHandle, 0, 0xffff, '0x2902');
    }).done(function (result) {
...
ccbnpDrivers.authenticate = function (periph, ioCap, mitm, bond) {
    ...
    ccbnp.gap.authenticate(periph.connHandle, ioCap, 0, new Buffer(16).fill(0), mitm | bond, 16, keyDist, 0, 0, 0, 0, 16, keyDist)
    .done(function (result) {
...
ccbnpDrivers.read = function (char) {
    ...
    ccbnp.gatt.readCharValue(char._service._peripheral.connHandle, char.handle, char.uuid).done(function (result) {
...
ccbnpDrivers.readDesc = function (char) {
    ...
    ccbnp.gatt.readUsingCharUuid(char._service._peripheral.connHandle, startHandle, endHandle, '0x2901').then(function (result) {
...
ccbnpDrivers.notify = function (char, config, oldDeferred) {
    var deferred = oldDeferred ? oldDeferred : Q.defer(),
        startHandle,
        endHandle;

    if (notifySpinlock) 
        notifyQueue.push({ char: char, config: config, deferred: deferred });
    else {
        notifySpinlock = true;
        ...
        ccbnp.gatt.readUsingCharUuid(char._service._peripheral.connHandle, startHandle, endHandle, '0x2902').then(function (result) {
            ...
            notifySpinlock = false;

            if (_.isEmpty(notifyQueue)) return;
            process.nextTick(function () {
                var nextNotify = notifyQueue.shift();
                ccbnpDrivers.notify(nextNotify.char, nextNotify.config, nextNotify.deferred);
            });
        }).done();
    }
