...
BShepherd.prototype.reset = function (callback) {
    ...
    ccbnp.hci.resetSystem(0).then(function () {
        console.log('stop!');
        return self.stop();
    }).then(function () {
        // setTimeout(function () {
            return self.start(function () {}, spCfg);
        // }, 3000);
...
/*    ccbnp.hci.resetSystem(0, function () {
        _.forEach(devmgr.bleDevices, function (periph) {
...
/*    ccbnp.hci.resetSystem(1, function (err) {
        if (!err) {
            setTimeout(function () {
                self.start(function () {}, spCfg);
             }, 5000);
        }
    });*/

  /*  ccbnp.hci.resetSystem(1).then(function () {
        return ccbnp.gap.deviceInit(0x0008, 5, new Buffer(16).fill(0), new Buffer(16).fill(0), 1);
