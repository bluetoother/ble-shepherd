...
manager.nwkScanner.on('NS:IND', nwkScannerEvtHdlr);
...
function bleInitDoneHdlr (msg) {
    ...
    try {
        central = manager.devmgr.newDevice('central', msg.devAddr);
        manager.bleCentral = central;
        manager.devmgr.bleDevices.push(central); 
    } catch (e) {
        console.log('Can not create central with error: ' + e + '.');
    }

    manager.appInit();
    manager.nwkScanner.getScanParams().then(function () {
        return manager.nwkScanner.getLinkParams();
    }).then(function () {
        return manager.nwkScanner.setScanParams({time: 3000});
    }).then(function () {
        console.log('>> Loading devices from database.');
        return manager.devmgr.loadDevs();
    }).then(function (devs) {
                    ...
                    manager.on('IND', function (msg) {
                        if (msg.type === 'DEV_INCOMING' && msg.data === dev.addr) {
                            ...
                            if (onlineNum === 3 && devCount !== _.size(devs)) { 
                                manager.devmgr.pauseDev().then(function (devAddr) {
        ...
        manager.nwkScanner.scan();
        manager.app();
        notifPeriphState(connDevs, pauseDevs);
...
function nwkScannerEvtHdlr (devsInfo) {
    var dev,
        discDevNum = 0;
    ...
    if (manager._permitState === 'on') {
        _.forEach(devsInfo, function (devInfo) {
            try {
                dev = manager.devmgr.newDevice('peripheral', devInfo.addr, devInfo.addrType);
...
function ccBnpEvtHdlr (msg) {
    ...
    try {
        if (msg.type === 'linkEstablished') {
            dev = manager.devmgr.findDev(msg.data.addr);
        } else if (msg.type !== 'attReq') {
            dev = manager.devmgr.findDev(msg.data.connHandle);
        }
    } catch (e) {
...
function processLinkEst (msg, dev) {
    ...
    if (!dev) {
        try {
            dev = manager.devmgr.newDevice('peripheral', msg.addr, 0);
        } catch (e) {
            console.log('Can not create a device with error: ' + e + '.');
        }
    }
...
function processLinkTerm (msg, dev) {
    ...
    manager.emit('IND', {type: 'DEV_LEAVING', data: dev.addr});
}
...
function connectDev (dev) {
    ...
    if (manager._connSpinLock === 'off' && manager._connNum < 3) {
        ...
        dev.connect().fail(function (err) {
            ...
            if (!_.isEmpty(manager.devmgr._discDevs)) {
                if (manager._connNum >= 3) { timeout = 2000; }
                setTimeout(function () {
                    process.nextTick(function () {
                        var nextDev = manager.devmgr._discDevs.shift();
                        connectDev(nextDev);
                    });
                }, timeout);
            } else {
        ...
        if (manager._connNum >= 3) {
            manager.devmgr.pauseDev().then(function (devAddr) {
                manager.emit('IND', {type: 'DEV_PAUSE', data: devAddr});
                connectDev(dev);
            }).fail(function (err) {
        ...
        } else if (!_.find(manager.devmgr._discDevs, {addr: dev.addr})) {
            manager.devmgr._discDevs.push(dev);
        }
    }
}
...
function syncDev (dev, flag) {
    function getDevInfo (rebuildFlag) {
        if (rebuildFlag === true) {
            return dev.getServs().then(function () {
                return dev.save();
            });
        } else {
            return dev.update();
    ...
    if (manager._syncSpinLock === 'off') {
        ...
        getDevInfo(flag).then(function () {
            ...
            manager.emit('IND', {type: 'DEV_INCOMING', data: dev.addr});
        }).fail(function (err) {
            console.log('Device: ' + dev.addr + ' update GATT information failure with error: ' + err +'.');
            dev.disconnect();
        }).finally(function () {
            manager._syncSpinLock = 'off';
            if (!_.isEmpty(manager.devmgr._syncDevs)) {
                if (_.size(manager.devmgr._discDevs !== 0)) {
                    manager.devmgr.pauseDev().then(function (devAddr) {
                        manager.emit('IND', {type: 'DEV_PAUSE', data: devAddr});
                    }).fail(function () {}).done();
                }
                process.nextTick(function () {
                    var nextDevInfo = manager.devmgr._syncDevs.shift();
                    syncDev(nextDevInfo.dev, nextDevInfo.flag);
                });
            }
        }).done();
    } else {
        manager.devmgr._syncDevs.push({dev: dev, flag: flag});
    }
}

function notifPeriphState(connDevs, pauseDevs) {
    _.forEach(connDevs, function (devAddr) {
        manager.emit('IND', {type: 'DEV_INCOMING', data: devAddr});
    });
