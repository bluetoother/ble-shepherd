...
BShepherd.prototype._regUuidHdlTable = function () {
    ...
    _.forEach(devmgr.bleDevices, function (dev) {
                    ...
                    manager.on('IND', function (msg) {
                        if (msg.type === 'DEV_INCOMING' && msg.data === dev.addr) {
                            ...
                            if (onlineNum === 3 && devCount !== _.size(devs)) { 
                                devmgr.pauseDev().then(function (devAddr) {
...
function scanHdlr (devsInfo) {
    ...
    _.forEach(devsInfo, function (devInfo) {
        if (oldPermitState === 'on') {
            dev = devmgr.newDevice('peripheral', devInfo.addr, devInfo.addrType);
        } else {
            dev = manager.find(devInfo.addr);
        }

        if (dev && dev.state !== 'pause') {
            if (!_.includes(manager.blackList, dev.addr)) {
                discDevNum += 1;
                connectPeriph(dev);
            }
        }
...
function ccBnpEvtHdlr (msg) {
    ...
    try {
        if (msg.type === 'linkEstablished') {
            dev = devmgr.findDev(msg.data.addr);
        } else if (msg.type !== 'attReq') {
            dev = devmgr.findDev(msg.data.connHandle);
        }
    } catch (e) {
...
function processLinkEst (msg, dev) {
    ...
    if (!dev) {
        try {
            dev = devmgr.newDevice('peripheral', msg.addr, 0);
        } catch (e) {
...
/*** Private Function                                                                          ***/
/*************************************************************************************************/
function connectPeriph (dev) {
    var timeout = 0;

    if (manager._connSpinLock === 'on') {
        if (!_.find(devmgr._discDevs, {addr: dev.addr})) {
            devmgr._discDevs.push(dev);
        }
    ...
    if (manager._syncSpinLock === 'off') {
        ...
        getDevInfo(flag).then(function () {
            ...
            if (!_.isEmpty(devmgr._syncDevs)) {
                process.nextTick(function () {
                    var nextDevInfo = devmgr._syncDevs.shift();
                    syncPeriph(nextDevInfo.dev, nextDevInfo.flag);
                });
            }
        }).done();
    } else {
        devmgr._syncDevs.push({dev: dev, flag: flag});
    }
}
