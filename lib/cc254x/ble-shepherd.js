...
BShepherd.prototype.start = function (bleApp, spCfg, callback) {
    ...
    if (!_.isPlainObject(spCfg) || _.isUndefined(spCfg.path)) {
        callback(new Error('spConfig must be an object and should have path property'));
    } else {
        ccbnp.init(spCfg, 'central', function (err, result) {
            if (err) {  
                callback(err);
            } else if (_.isFunction(bleApp)) {
                self.app = bleApp;
...
BShepherd.prototype.stop = function (callback) {
    ...
    if (!this._enable) {
        ...
        bleutil.seqResolveQFuncs(disconnPeriphs).done(function (result) {
            self._enable = false;
            callback(null);
        }, function (err) {
            callback(err);
...
BShepherd.prototype.reset = function (mode, callback) {
    ...
    if (mode !== 'hard' && mode !== 'soft') {
        callback(new Error('mode must be a string of soft or hard'));
    } else {
        resetMode = (mode === 'hard') ? 0 : 1;
        ccbnp.hci.resetSystem(resetMode, function (err) {
            callback(err);
        });
    }
};
...
function initDoneHdlr (msg, callback) {
    ...
    if (manager._permitState === 'pending') manager.appInit();
