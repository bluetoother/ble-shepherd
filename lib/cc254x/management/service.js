...
Service.prototype._getChars = function (callback) {
    var self = this,
        deferred = Q.defer(),
        charObj,
        charsInfo = [],
        ...
        ccBnp.gatt.discAllChars(this.ownerDev.connHdl, this.startHdl, this.endHdl).then(function (result) {
            ...
            _.forEach(charsInfo, function (charInfo) {
                ...
                _.forEach(GATTDEFS.Prop._enumMap, function (propVal, propName) {
                    if (charInfo.prop & propVal) { prop.push(propName); }
                });
                charInfo.prop = prop;
                prop = [];
...
Service.prototype._loadChars = function (callback) {
    var self = this,
        deferred = Q.defer(),
        char;
...
Service.prototype.remove = function (callback) {
    ...
    bledb.remove('service', this._id).then(function () {
        devServs.splice(_.indexOf(devServs, self), 1);
        deferred.resolve();
    }).fail(function (err) {
        deferred.reject(err);
    }).done();
