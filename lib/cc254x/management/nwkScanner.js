...
NwkScanner.prototype.getScanParams = function () {
    ...
    ccbnp.gap.getParam(2).then(function (result) {
        self.scanParams.time = result.payload.readUInt16LE(0);
        return ccbnp.gap.getParam(16);
    }).then(function (result) {
        self.scanParams.interval = result.payload.readUInt16LE(0);
        return ccbnp.gap.getParam(17);
    }).then(function (result) {
        self.scanParams.window = result.payload.readUInt16LE(0);
...
NwkScanner.prototype.getLinkParams = function () {
    ...
    ccbnp.gap.getParam(21).then(function (result) {
        self.linkParams.interval = result.payload.readUInt16LE(0);
        return ccbnp.gap.getParam(26);
    }).then(function (result) {
        self.linkParams.latency = result.payload.readUInt16LE(0);
        return ccbnp.gap.getParam(25);
    }).then(function (result) {
        self.linkParams.timeout = result.payload.readUInt16LE(0);
        deferred.resolve();
    }).fail(function (err) {
        deferred.reject(err);
    }).done();
...
NwkScanner.prototype.scan = function () {
    ...
    ccbnp.gap.deviceDiscReq(3, 1, 0).then(function (result) {
        var devDisc = result.collector.GapDeviceDiscovery[0];
        devs = devDisc.devs;
        devs = devs || [];
