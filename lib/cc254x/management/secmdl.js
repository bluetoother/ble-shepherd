...
Secmdl.prototype.passPasskey = function (passkey, callback) {
    ...
    if (!_.isNumber(passkey) || _.size(passkey.toString()) > 6) {
        ...
        ccBnp.gap.passkeyUpdate(this.ownerDev.connHdl, passkeyStr).then(function () {
            deferred.resolve();
        }).fail(function(err) {
            deferred.reject(err);
        }).done();
...
Secmdl.prototype.bonding = function (callback) {
    ...
    if (!this.ltk || !this.div || !this.rand) {
        ...
        ccBnp.gap.bond(this.ownerDev.connHdl, mitm, this.ltk, this.div, this.rand, this.ltk.length).then(function () {
            deferred.resolve();
        }).fail(function (err) {
            deferred.reject(err);
        }).done();
