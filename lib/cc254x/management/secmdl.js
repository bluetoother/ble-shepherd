...
Secmdl.prototype.passPasskey = function (passkey, callback) {
    ...
    if (!_.isNumber(passkey) || _.size(passkey) !== 6) {
        deferred.reject(new Error('Passkey must be a string of length 6.'));
    } else {
        ccBnp.gap.passkeyUpdate(this._ownerDev.connHdl, passkey).then(function () {
            deferred.resolve();
        }).fail(function(err) {
            deferred.reject(err);
        }).done();
