...
function nwkScannerEvtHdlr (msg) {
    ...
    if (discDevNum !== 0) {
        ...
        scanEmptyCount += 1;
        // processScanTimeChange(scanEmptyCount).done(function () {
            bShepherd.nwkScanner.permitJoin(true);
            bShepherd.devmgr._scanState = 'on';
        // });
    }
...
function processScanTimeChange (times, callback) {
    var deferred = Q.defer(),
        scanTime;
console.log(times);
    if (times === 1) {
    ...
    if (scanTime) {
        console.log('start to change scanTime');
        bShepherd.nwkScanner.setScanParams({time: scanTime}).then(function () {
            console.log('change!!!');
            deferred.resolve();
        }).fail(function (err) {
            console.log(err);
