            ...
            if (bShepherd.nwkScanner.permitState === 'on') {
                ...
                _.forEach(devsInfo, function (devInfo) {
                    try {
                        ...
                        if (!_.startsWith(dev.addr, '0xd05fb820') && dev.addr !== '0x5c313e2bfb7b') {  // && dev.addr === '0x78c5e570796e'
                            discDevNum += 1;
                            connectDev(dev); 
                        }
                    } catch (e) {
...
function connectDev (dev) {
    if (bShepherd._connSpinLock === 'off') {
        ...
        if (!_.find(bShepherd.devmgr._discDevs, {addr: dev.addr})) {
