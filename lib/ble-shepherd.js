...
BShepherd.prototype.addLocalServ = function (servInfo, callback) {
    ...
    if (!_.isPlainObject(servInfo)) { checkErr = new TypeError('servInfo must be an object'); }
    if (!_.isArray(servInfo.charsInfo)) { checkErr = new TypeError('servInfo.charsInfo must be an array.'); }
    if (!_.isString(servInfo.uuid) | !_.startsWith(servInfo.uuid, '0x')) {  
        checkErr = new TypeError('servInfo.uuid must be a string and start with 0x');
    }
...
BShepherd.prototype.findDev = function (addrOrHdl) {
    ...
    if (msg.type === 'STATE_CHANGE') {
        ...
        _.forEach(devsInfo, function (devInfo) {
            try {
                ...
                if (dev.state === 'disc' && dev.addr !== '0xd05fb820a857' && dev.addr !== '0x5c313e2bfb7b') {  // && dev.addr === '0x78c5e570796e'
                    discDevNum += 1;
                    connectDevice(dev); 
                }
            } catch (e) {
