...
Blemgr.prototype.regUuidHdlTable = function () {
    var table = {};
...
function bleInitDoneHdlr (msg) {
    ...
    blemgr.devmgr.loadDevs().then(function (devs) {
        ...
        blemgr.regUuidHdlTable();
        blemgr.nwkScanner.permitJoin(true);
        blemgr.devmgr._scanState = 'on';
        blemgr.app();
...
function nwkScannerEvtHdlr (msg) {
    ...
    blemgr.devmgr._scanState = 'off';
    _.forEach(devsInfo, function (devInfo) {
        dev = blemgr.devmgr.newDevice('peripheral', devInfo.addr, devInfo.addrType);
        if (dev.state === 'disc' && (dev.addr === '0x78c5e570796e' || dev.addr === '0x9059af0b7722')) {  // && dev.addr === '0x78c5e570796e'
            discDevNum += 1;
            connectDevice(dev); 
        }
    });
...
function processAttrInd (type, msg) {
    ...
    blemgr.emit('ind', {type: 'ATT_IND', data: msg});
}
