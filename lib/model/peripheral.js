...
Peripheral.prototype.findServ = function (sid) {
    sid = butil.checkUuid(sid, 'sid must be a number or a string');

    if (_.isNumber(sid))
        return this.servs[sid];
    else if (_.isString(sid)) 
...
Peripheral.prototype.findChar = function (sid, cid) {
    ...
    if (serv) {
        cid = butil.checkUuid(cid, 'cid must be a number or a string');

        if (_.isNumber(cid)) 
            return serv.chars[cid];
        else if (_.isString(cid)) 
...
Peripheral.prototype.read = function (sid, cid, callback) {
    ...
    return this._wakeUp().then(function () {
        ...
        proving.defined(char, 'Can not find characteristic.');
...
Peripheral.prototype.write = function (sid, cid, value, withoutResponse, callback) {
    var self = this,
        char;
    ...
    if (_.isFunction(withoutResponse)) {
        callback = withoutResponse;
    ...
    return this._wakeUp().then(function () {
        ...
        return char.write(value, withoutResponse);
    }).then(function (result) {
        self._cmdCount += 1;
    }).nodeify(callback);
};
...
Peripheral.prototype.configNotify = function (sid, cid, config, callback) {
    ...
    proving.boolean(config, 'config must be a boolean');
...
Peripheral.prototype.onNotified = function (sid, cid, fn) {
    ...
    proving.fn(fn, 'fn must be a function');
