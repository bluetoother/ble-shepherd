...
BShepherd.prototype.list = function (addrs) {
    ...
    if (!_.isUndefined(addrs) && !_.isArray(addrs))
        throw new TypeError('addrs should be a string or an array of strings if given.');

    if (!addrs) 
        foundPeriphs = _.map(periphs, function (periph) {
            return {
                addr: periph.addr,
                addrType: periph.addrType,
                status: periph.status,
                connHandle: periph.connHandle,
                servList: getServList(periph)
            };
        });
    else if (_.isArray(addrs)) 
        _.forEach(addrs, function (addr) {
            var rec,
                found = self.find(addr);

            if (found) {
                foundPeriphs.push({
                    addr: found.addr,
                    addrType: found.addrType,
                    status: found.status,
                    connHandle: found.connHandle,
                    servList: getServList(found)
                });
            }
        });
...
BShepherd.prototype.remove = function (addrOrHdl, callback) {
    ...
    rmvAndEmit = function (deferred) {
        self._periphBox.remove(periph._id, function (err) {
            if (err) {
                ...
                self.emit('ind', { type: 'devLeaving', data: periph.addr });
                deferred.resolve();
            }
        });
    };
    ...
    if (!periph) 
        deferred.reject('Peripheral not exist');
    else {
...
BShepherd.prototype.declare = function (type, regObjs) {
    ...
    _.forEach(regObjs, function (regObj) {
        ...
        var existName = GATTDEFS[enumName].get(regObj.name),
            existUuid = GATTDEFS[enumName].get(regObj.uuid);
