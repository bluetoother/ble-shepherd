...
function BShepherd (subModule, path, opts) {
    ...
    var onSignin = function () {
            ...
            sigIntListeners.forEach(function (sigIntLsn) {
                if (sigIntLsn === onSignin)
                    async.exit();
            });
        },
...
BShepherd.prototype.onDiscovered = function (hdlr) {
    ...
    this.acceptDevIncoming = function (periphInfo, cb) {
        var pInfo = {addr: periphInfo.addr, advertisement: periphInfo.advertisement};

        hdlr(pInfo, function (err, accepted) {
            if (err)
                cb(err);
            else
...
BShepherd.prototype._examinePeriph = function (periph) {
    ...
    _.forEach(plugins, function (plgInfo, periphName) {
        ...
        if (periph.findChar('0x180a', '0x2a29'))
            basicInfo.manufacturer = periph.findChar('0x180a', '0x2a29').value.manufacturerName;
        if (periph.findChar('0x180a', '0x2a24'))
            basicInfo.model = periph.findChar('0x180a', '0x2a24').value.modelNum;
        if (periph.findChar('0x180a', '0x2a25'))
            basicInfo.serial = periph.findChar('0x180a', '0x2a25').value.serialNum;
        if (periph.findChar('0x180a', '0x2a26'))
            basicInfo.fwRev = periph.findChar('0x180a', '0x2a26').value.firmwareRev;
        if (periph.findChar('0x180a', '0x2a27'))
            basicInfo.hwRev = periph.findChar('0x180a', '0x2a27').value.hardwareRev;
        if (periph.findChar('0x180a', '0x2a28'))
            basicInfo.swRev = periph.findChar('0x180a', '0x2a28').value.softwareRev;
